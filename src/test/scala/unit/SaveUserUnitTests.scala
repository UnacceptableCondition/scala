...
class SaveUserUnitTests extends AnyFreeSpec with ArgumentMatchersSugar with TestHelperAliases with TestsUtilFunctions {
  ...
  "UserService" - {
    ...
    "SaveUser." +
      ...
      "If Settings.defaultUserIsActive == false - insertUser and insertUserActivity actions should be invoked only once" in {
        ...
        Mockito.verify(userRep, times(1)).insertAction(*[User])
        Mockito.verify(userToGroupRep, times(0)).insertAction(*[UserToGroupEntity])
        Mockito.verify(userActivityRep, times(1)).insertAction(*[UserActivityEntity])
      }
    }
  }
